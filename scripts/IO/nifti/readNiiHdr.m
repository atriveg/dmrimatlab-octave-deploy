function [hdr, filetype, fileprefix, machine] = readNiiHdr(fileprefix,exttype,gzipfile)

machine = 'ieee-le';

if(gzipfile)
    fileprefix(end-2:end)='';
end
...
if(exttype)
    ...
    filename = sprintf('%s.hdr',fileprefix);
end
...
if fread(fid,1,'int32') == 348
    ...
    if fread(fid,1,'int32') ~= 348
        fclose(fid);
        error('File "%s" is corrupted (wrong header size).',filename);
    end
...
% fseek cannot be used with gzipped files, so let's forget about
% the originator:
%fseek(fid,253,'bof');
